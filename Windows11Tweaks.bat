...
rem ::: !!! WARNING !!!
rem ::: !!! May cause unstable system or crashes !!!
rem ::: !!! Use script as reference only and modify to your own experience !!!
...
rem ::: Show Sync Provider Notifications in Windows Explorer (ADs)
...
rem ::: Do this for all current items checkbox / 1 - Disabled
...
rem ::: Always show more details in copy dialog
...
reg add "HKCU\Control Panel\Desktop" /v "MenuShowDelay" /t REG_SZ /d "500" /f

rem ::: Mouse Hover Time in milliseconds before Pop-up Display
reg add "HKCU\Control Panel\Mouse" /v "MouseHoverTime" /t REG_SZ /d "500" /f

rem ::: Disable Windows caching DLL in memory
...
rem ::: Background disk defragmentation n - Disable  / y - enable
reg add "HKLM\Software\Microsoft\Dfrg\BootOptimizeFunction" /v "Enable" /t REG_SZ /d "n" /f

rem ::: Disable FTH (Fault Tolerant Heap)
reg add "HKLM\Software\Microsoft\FTH" /v "Enabled" /t Reg_DWORD /d "0" /f

rem ::: Disable Background auto-layout / Disable Optimize Hard Disk when idle
...
rem ::: Enables 8dot3 name creation for all volumes on the system / 1 - Disables 8dot3 name creation for all volumes on the system / 2 - Sets 8dot3 name creation on a per volume basis / 3 - Disables 8dot3 name creation for all volumes except the system volume
rem ::: fsutil 8dot3name scan c:\
fsutil behavior set disable8dot3 1

rem ::: When listing directories, NTFS does not update the last-access timestamp, and it does not record time stamp updates in the NTFS log
rem ::: fsutil behavior query disablelastaccess
fsutil behavior set disablelastaccess 1

rem ::: Raise the limit of paged pool memory / 1 - Default
fsutil behavior set memoryusage 2

rem ::: Default / 1 - On / 2 - Off
reg add "HKLM\System\CurrentControlSet\Control\DeviceGuard" /v "ConfigureSystemGuardLaunch" /t REG_DWORD /d "2" /f

rem ::: Enable virtualization-based security / run msinfo32 to check
reg add "HKLM\System\CurrentControlSet\Control\DeviceGuard" /v "EnableVirtualizationBasedSecurity" /t REG_DWORD /d "0" /f
reg add "HKLM\System\CurrentControlSet\Control\DeviceGuard\Scenarios\HypervisorEnforcedCodeIntegrity" /v "Enabled" /t REG_DWORD /d "0" /f

rem ::: Drivers and the kernel can be paged to disk as needed / 1 - Drivers and the kernel must remain in physical memory
reg add "HKLM\System\CurrentControlSet\Control\Session Manager\Memory Management" /v "DisablePagingExecutive" /t REG_DWORD /d "1" /f

rem ::: Establishes a standard size file-system cache of approximately 8 MB / 1 - Establishes a large system cache working set that can expand to physical memory, minus 4 MB, if needed
reg add "HKLM\System\CurrentControlSet\Control\Session Manager\Memory Management" /v "LargeSystemCache" /t REG_DWORD /d "1" /f

rem ::: Disable It / 1 - Default
...
rem ::: Disable Fast Startup for a Full Shutdown / 1 - Enable Fast Startup (Hybrid Boot) for a Hybrid Shutdown
...
rem ::: Disable Low Disk Space Alerts
...
rem ::: Disable Microsoft Windows Just-In-Time (JIT) script debugging
...
rem ::: Do not add shares from recently opened documents to the My Network Places folder
reg add "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "Norecentdocsnethood" /t REG_DWORD /d "1" /f

rem ::: Disable / 1 - Enable (Default)
...
rem ::: Disable Steps Recorder (Steps Recorder keeps a record of steps taken by the user, the data includes user actions such as keyboard input and mouse input user interface data and screen shots)
...
rem ::: Play Windows Startup sound
...
rem ::: Show text suggestions when typing on the physical keyboard (Privacy)
reg add "HKCU\Software\Microsoft\Input\Settings" /v "EnableHwkbTextPrediction" /t REG_DWORD /d "0" /f

rem ::: Typing insights (Privacy)
reg add "HKCU\Software\Microsoft\Input\Settings" /v "InsightsEnabled" /t REG_DWORD /d "0" /f

rem ::: Multilingual text suggestions (Privacy)
...
rem ::: Automatically save my restartable apps when I sign out and restart them after I sign in
...
rem ::: Download over metered connections
...
rem ::: Get fun facts, tips, and more from Windows and Cortana on your lock screen (Windows spotlight)
...
rem ::: Show flashing on taskbar apps
...
rem ::: Let websites show me locally relevant content by accessing my language list (let browsers access your local language)
reg add "HKCU\Control Panel\International\User Profile" /v "HttpAcceptLanguageOptOut" /t REG_DWORD /d "1" /f

rem ::: Let Windows improve Start and search results by tracking app launches (Remember commands typed in Run) / 0 - Disable and Disable "Show most used apps"
...
rem ::: Enable and Prioritize Edge Notifications by not showing them in the notification center 
...
rem ::: Antivirus Disabled Notification
reg add "HKLM\Software\Microsoft\Windows Defender Security Center\Notifications" /v "DisableNotifications" /t REG_DWORD /d "1" /f
reg add "HKLM\Software\Policies\Microsoft\Windows Defender Security Center\Notifications" /v "DisableEnhancedNotifications" /t REG_DWORD /d "1" /f

rem ::: Security and Maitenance Notification
...
rem ::: Foreground and background applications equally responsive / 1 - Foreground application more responsive than background / 2 - Best foreground application response time (Default)
...
rem ::: Clipboard history
reg add "HKLM\Software\Policies\Microsoft\Windows\System" /v "AllowClipboardHistory" /t REG_DWORD /d "0" /f

rem ::: Sync across your devices
reg add "HKLM\Software\Policies\Microsoft\Windows\System" /v "AllowCrossDeviceClipboard" /t REG_DWORD /d "0" /f

rem ::: Suggested actions
...
rem ::: Show me the Windows welcome experience
...
rem ::: Disable search box options
...
rem ::: reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "NoWinKeys" /t REG_DWORD /d "0" /f
...
rem ::: reg add "HKCU\Software\Microsoft\GameBar" /v "AllowAutoGameMode" /t REG_DWORD /d "0" /f
rem ::: reg add "HKCU\Software\Microsoft\GameBar" /v "AutoGameModeEnabled" /t REG_DWORD /d "0" /f
...
rem ::: https://www.geoffchappell.com/studies/windows/win32/services/svchost/index.htm
reg add "HKLM\SYSTEM\CurrentControlSet\Services\Dnscache\Parameters" /v "ServiceDllUnloadOnStop" /t REG_DWORD /d "0" /f
...
rem ::: Enable Windows automatically restarting after a system crash
...
rem ::: Disable LMHOSTS Lookup on all adapters / 1 - Enable
reg add "HKLM\System\CurrentControlSet\Services\NetBT\Parameters" /v "EnableLMHOSTS" /t REG_DWORD /d "0" /f

rem ::: Disable NetBIOS over TCP/IP on all adapters / 1 - Enable / 0 - Default
...
rem ::: GameDVR Disable Fullscreen Optimizations (FSO)
...
rem ::: GameDVR Disable more FSO, removes Color Management
rem ::: 0 = Enable FSO / 1 = Disable FSO
reg add "HKCU\System\GameConfigStore" /v "GameDVR_DXGIHonorFSEWindowsCompatible" /t REG_DWORD /d "0" /f

rem ::: GameDVR Honor User adjusted DSE value
...
rem ::: Enable Scroll Inactive Windows
reg add "HKCU\Control Panel\Desktop" /v "MouseWheelRouting" /t REG_DWORD /d "1" /f
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\AppPrivacy" /v "LetAppsAccessCalendar" /t REG_DWORD /d "1" /f
...
rem ::: reg add "HKCU\Control Panel\Desktop" /v "DragFromMaximize" /t REG_DWORD /d "0" /f
...
reg add "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "NoThumbNailCache" /t REG_DWORD /d "0" /f
...
rem ::: Disable Driver Updates through Windows Update and Online Metadata
